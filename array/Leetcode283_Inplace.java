...
public class Leetcode283_Inplace {
    public void moveZeroes(int[] nums) {
        int j = 0;
        int temp = 0; 
        double lowerlimit = Math.pow(-2, 31);
        double upperlimit = Math.pow(2, 31);
        if(1 <= nums.length && nums.length <= 10000) {
            for(int i = 1; i < nums.length; i++) {
                if(lowerlimit <= nums[i] && nums[i] <= upperlimit - 1) {
                    if(Math.abs(nums[j]) == 0 && (Math.abs(nums[j]) < Math.abs(nums[i]))) {
                        temp = Math.abs(nums[j]);
                        nums[j] = Math.abs(nums[i]);
                        nums[i] = temp;
                        j++;
                    }
                    else if(nums[i] == 0 && Math.abs(nums[j]) == Math.abs(nums[i])){
                        continue;
                    }
                    else {
                        j++;
    ...
    public static void main(String[] args) {
        ...
        Leetcode283_Inplace obj = new Leetcode283_Inplace();
        obj.moveZeroes(nums);
    }
}
